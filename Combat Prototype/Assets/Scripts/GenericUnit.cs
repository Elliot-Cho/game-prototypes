...
public class GenericUnit : Unit
{
    ...
    public override void MarkAsAttacking(MonoBehaviour other)
    {
    ...
    public override void MarkAsHealing(Unit other)
    {
        StartCoroutine(Glow(new Color(0, 1, 0, 0.5f), 1));
    }
    public override void MarkAsDestroyed()
    {
    }

    private IEnumerator Jerk(MonoBehaviour other)
    {
    ...
    {
        var _renderer = transform.GetComponent<SpriteRenderer>();
        float startTime = Time.time;

        while (startTime + cooloutTime > Time.time)
        {
            _renderer.color = Color.Lerp(new Color(1,1,1,1), color, (startTime + cooloutTime) - Time.time);
            yield return 0;
        }
